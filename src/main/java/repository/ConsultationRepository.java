...
public class ConsultationRepository {
    ...
    public Consultation save(Consultation consultation) throws SQLException {
        String sql = "INSERT INTO Consultation " +
                "(appointmentRequestId, clientId, doctorId, notes, diagnosis, prescription, consultationDate, creationDate) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<Consultation> findAll() throws SQLException {
        ...
        String sql = "SELECT * FROM Consultation ORDER BY creationDate DESC";
        Connection conn = getConnection();
    ...
    public List<Consultation> findByClientId(long clientId) throws SQLException {
        List<Consultation> list = new ArrayList<>();
        String sql = "SELECT * FROM Consultation WHERE clientId = ? ORDER BY creationDate DESC";
        Connection conn = getConnection();
    ...
    public List<Consultation> findByDoctorId(long doctorId) throws SQLException {
        List<Consultation> list = new ArrayList<>();
        String sql = "SELECT * FROM Consultation WHERE doctorId = ? ORDER BY creationDate DESC";
        Connection conn = getConnection();
    ...
    public List<Consultation> findByAppointmentRequestId(long appointmentRequestId) throws SQLException {
        List<Consultation> list = new ArrayList<>();
        String sql = "SELECT * FROM Consultation WHERE appointmentRequestId = ? ORDER BY creationDate DESC";
        Connection conn = getConnection();

        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setLong(1, appointmentRequestId);
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next())
                    list.add(mapRow(rs));
            }
    ...
    public void update(Consultation consultation) throws SQLException {
        String sql = "UPDATE Consultation " +
                "SET appointmentRequestId=?, clientId=?, doctorId=?, notes=?, " +
                "    diagnosis=?, prescription=?, consultationDate=? " +
                "WHERE id=?";
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.setString   (6, consultation.getPrescription());
            pstmt.setTimestamp(7, Timestamp.valueOf(consultation.getConsultationDate()));
            pstmt.setLong     (8, consultation.getId());
            pstmt.executeUpdate();
        }
    }
